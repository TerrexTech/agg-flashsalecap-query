...
func queryFlashsale(collection *mongo.Collection, event *model.Event, findopts ...findopt.Find) *model.KafkaResponse {
	...
	if err != nil {
		err = errors.Wrap(err, "Query: Error marshalling Flashsale Delete-result")
		log.Println(err)
		return &model.KafkaResponse{
			AggregateID:   event.AggregateID,
			CorrelationID: event.CorrelationID,
